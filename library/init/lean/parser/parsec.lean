...
@[specialize] def take_until (p : char → bool) : m string :=
take_while (λ c, !p c)

@[specialize] def take_until1 (p : char → bool) : m string :=
take_while1 (λ c, !p c)
...
@[inline] def many1 [alternative m] (p : m α) : m (list α) :=
do r ← remaining, many1_aux p r

@[inline] def many [alternative m] (p : m α) : m (list α) :=
many1 p <|> pure []
...
@[inline] def many' [alternative m] (p : m α) : m unit :=
many1' p <|> pure ()
...
@[specialize] def longest_match [monad_except (message μ) m] (ps : list (m α)) : m (list α) :=
do it ← left_over,
...
@[specialize] def observing [monad_except (message μ) m] (p : m α) : m (except (message μ) α) :=
...
@[specialize] def parse (p : parsec_t μ m α) (s : string) (fname := "") : m (except (message μ) α) :=
run p s fname

@[specialize] def parse_with_eoi [inhabited μ] (p : parsec_t μ m α) (s : string) (fname := "") : m (except (message μ) α) :=
run (p <* eoi) s fname

@[specialize] def parse_with_left_over [inhabited μ] (p : parsec_t μ m α) (s : string) (fname := "") : m (except (message μ) (α × iterator)) :=
run (prod.mk <$> p <*> left_over) s fname

end parsec_t

@[specialize] def parsec.parse {α : Type} (p : parsec μ α) (s : string) (fname := "") : except (message μ) α :=
