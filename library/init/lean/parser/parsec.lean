...
instance : has_append expected :=
⟨expected.append⟩

def expected.to_list : expected → list string
| expected.empty          := []
| (expected.singleton s)  := [s]
| (expected.merge s₁ s₂) := expected.to_list s₁ ++ expected.to_list s₂
...
(expected   : expected  := expected.empty) -- expected productions
(custom     : option μ)
...
@[inline_if_reduce] def bind_mk_res (ex₁ : option expected) (r : result μ β) : result μ β :=
match ex₁, r with
...
{expected := expected.singleton exp, ..msg}

@[inline_if_reduce] def labels_mk_res (r : result μ α) (lbls : expected) : result μ α :=
match r with
  | ok a it (some _) := ok a it (some lbls)
  | error msg ff     := error {expected := lbls, ..msg} ff
  | other            := other

@[inline] def labels (p : parsec_t μ m α) (lbls : expected) : parsec_t μ m α :=
λ it, do
  r ← p it,
...
def error {α : Type} (unexpected : string) (expected : expected := expected.empty)
...
-/
def str_core (s : string) (ex : expected) : m string :=
...
str_core s (expected.singleton $ repr s)
...
do it ← left_over,
   ...
   else error "end of input" (expected.singleton ("at least " ++ to_string n ++ " characters"))
...
do it ← left_over,
   ...
   if b then pure () else error msg expected.empty it

def eoi : m unit :=
do it ← left_over,
   if it.remaining = 0 then pure ()
   else error (repr it.curr) (expected.singleton "end of input")
