  ...
  unless df.builtinName.isAnonymous do
    registerBuiltinAttribute {
      ...
      add   := fun declName stx kind => do
        unless kind == AttributeKind.global do throw_error "invalid attribute '{df.builtinName}', must be global"
        let key ← df.evalKey true stx
        let decl ← getConstInfo declName
        match decl.type with
        | Expr.const c _ _ =>
          if c != df.valueTypeName then throw_error "unexpected type at '{declName}', '{df.valueTypeName}' expected"
          else
            let env ← getEnv
            let env ← declareBuiltin df attrDeclName env key declName
            setEnv env
        | _ => throw_error "unexpected type at '{declName}', '{df.valueTypeName}' expected",
