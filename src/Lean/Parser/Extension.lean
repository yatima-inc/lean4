...
private def BuiltinParserAttribute.add (attrName : Name) (catName : Name)
  ...
  unless kind == AttributeKind.global do throw_error "invalid attribute '{attrName}', must be global"
  ...
  | Expr.const `Lean.Parser.Parser _ _ => do
    ...
    setEnv env
  | _ => throw_error "unexpected parser type at '{declName}' (`Parser` or `TrailingParser` expected)"
...
private def ParserAttribute.add (attrName : Name) (catName : Name) (declName : Name) (stx : Syntax) (attrKind : AttributeKind) : AttrM Unit := do
  ...
  tokens.forM fun token => do
    try
      ...
      | Exception.error ref msg => throw_error "invalid parser '{declName}', {msg}"
      | ex => throw ex
