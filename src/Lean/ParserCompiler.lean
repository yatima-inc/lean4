...
partial def compileParserExpr (e : Expr) : MetaM Expr := do
  ...
  | _ => do
    ...
    let Expr.const c _ _ ← pure fn
      | throw_error "call of unknown parser at '{e}'"
    ...
    | none   =>
      ...
      if resultTy.isConstOf `Lean.Parser.TrailingParser || resultTy.isConstOf `Lean.Parser.Parser then do
        ...
        let some value ← pure cinfo.value?
          | throw_error "don't know how to generate {ctx.varName} for non-definition '{e}'"
        unless (env.getModuleIdxFor? c).isNone || force do
          throw_error "refusing to generate code for imported parser declaration '{c}'; use `@[runParserAttributeHooks]` on its definition instead."
        ...
        let some e' ← unfoldDefinition? e
          | throw_error "don't know how to generate {ctx.varName} for non-parser combinator '{e}'"
        compileParserExpr e'
end
