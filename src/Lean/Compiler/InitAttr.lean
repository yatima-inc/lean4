  ...
  registerParametricAttribute {
    ...
    getParam := fun declName stx => do
      ...
      | some initFnName =>
        ...
        | none => throw_error "initialization function '{initFnName}' must have type of the form `IO <type>`"
        | some initTypeArg =>
          if decl.type == initTypeArg then pure initFnName
          else throw_error "initialization function '{initFnName}' type mismatch"
      | none =>
