...
def getMVarDecl (mvarId : MVarId) : MetaM MetavarDecl := do
  ...
  | some d => pure d
  | none   => throw_error "unknown metavariable '{mkMVar mvarId}'"
...
def isReadOnlyLevelMVar (mvarId : MVarId) : MetaM Bool := do
  ...
  | _          => throw_error "unknown universe metavariable '{mkLevelMVar mvarId}'"
...
def throwUnknownFVar (fvarId : FVarId) : MetaM α :=
  throw_error "unknown free variable '{mkFVar fvarId}'"
...
def getLocalDeclFromUserName (userName : Name) : MetaM LocalDecl := do
  ...
  | some d => pure d
  | none   => throw_error "unknown local declaration '{userName}'"
