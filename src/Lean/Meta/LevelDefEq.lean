...
def decLevel (u : Level) : MetaM Level := do
  ...
  | some u => return u
  | none   => throw_error "invalid universe level, {u} is not greater than 0"
...
@[specialize] def withoutPostponingUniverseConstraintsImp {α} (x : MetaM α) : MetaM α := do
  ...
  try
    ...
    unless (← processPostponed (mayPostpone := false)) do
      throw_error "stuck at solving universe constraints{MessageData.nestD (postponedToMessageData (← getPostponed))}"
    setPostponed postponed
    return a
  catch ex =>
    setPostponed postponed
