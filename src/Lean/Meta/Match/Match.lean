...
private def throwInductiveTypeExpected {α} (e : Expr) : MetaM α := do
  let t ← inferType e
  throw_error "failed to compile pattern matching, inductive type expected{indentExpr e}\nhas type{indentExpr t}"
  ...
  | x :: xs => withGoalOf p do
    ...
    | some (ctorVal, xArgs) =>
      let alts ← p.alts.filterMapM fun alt => match alt.patterns with
        ...
        | p :: _  => throw_error "failed to compile pattern matching, inaccessible pattern or constructor expected{indentD p.toMessageData}"
        | _       => unreachable!
      ...
      let alts ← p.alts.filterMapM fun alt => match alt.patterns with
        | Pattern.inaccessible e :: ps => do
          if (← isDefEq x e) then
            ...
            pure none
        | p :: _ => throw_error "failed to compile pattern matching, unexpected pattern{indentD p.toMessageData}\ndiscriminant{indentExpr x}"
        | _      => unreachable!
      pure { p with alts := alts, vars := xs }
...
private def throwNonSupported (p : Problem) : MetaM Unit :=
  withGoalOf p do
    ...
    throw_error "failed to compile pattern matching, stuck at{indentD msg}"
...
def MatcherApp.addArg (matcherApp : MatcherApp) (e : Expr) : MetaM MatcherApp :=
  lambdaTelescope matcherApp.motive fun motiveArgs motiveBody => do
    unless motiveArgs.size == matcherApp.discrs.size do
      ...
      throw_error "unexpected matcher application, motive must be lambda expression with #{matcherApp.discrs.size} arguments"
    let eType ← inferType e
