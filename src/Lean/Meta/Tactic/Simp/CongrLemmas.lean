...
def mkCongrLemma (declName : Name) (prio : Nat) : MetaM CongrLemma := withReducible do
  ...
  match type.eq? with
  | none => throw_error "invalid 'congr' lemma, equality expected{indentExpr type}"
  | some (_, lhs, rhs) =>
    lhs.withApp fun lhsFn lhsArgs => rhs.withApp fun rhsFn rhsArgs => do
      unless lhsFn.isConst && rhsFn.isConst && lhsFn.constName! == rhsFn.constName! && lhsArgs.size == rhsArgs.size do
        throw_error "invalid 'congr' lemma, equality left/right-hand sides must be applications of the same function{indentExpr type}"
      let mut foundMVars : NameSet := {}
      for lhsArg in lhsArgs do
        unless lhsArg.isSort do
          unless lhsArg.isMVar do
            throw_error "invalid 'congr' lemma, arguments in the left-hand-side must be variables or sorts{indentExpr lhs}"
            ...
            | some (_, xLhs, xRhs) =>
              ...
              for y in ys do
                ...
                unless onlyMVarsAt yType foundMVars do
                  throw_error "invalid 'congr' lemma, argument #{j+1} of parameter #{i+1} contains unresolved parameter{indentExpr yType}"
                j := j + 1
              unless onlyMVarsAt xLhs foundMVars do
                throw_error "invalid 'congr' lemma, parameter #{i+1} is not a valid hypothesis, the left-hand-side contains unresolved parameters{indentExpr xLhs}"
              let xRhsFn := xRhs.getAppFn
              unless xRhsFn.isMVar do
                throw_error "invalid 'congr' lemma, parameter #{i+1} is not a valid hypothesis, the right-hand-side head is not a metavariable{indentExpr xRhs}"
              unless !foundMVars.contains xRhsFn.mvarId! do
                throw_error "invalid 'congr' lemma, parameter #{i+1} is not a valid hypothesis, the right-hand-side head was already resolved{indentExpr xRhs}"
              for arg in xRhs.getAppArgs do
                unless arg.isFVar do
                  throw_error "invalid 'congr' lemma, parameter #{i+1} is not a valid hypothesis, the right-hand-side argument is not local variable{indentExpr xRhs}"
              pure (some xRhsFn)
          match rhsFn? with
          | none       => pure ()
          | some rhsFn =>
