...
private def throwInductiveTypeExpected {α} (type : Expr) : MetaM α := do
  throw_error "failed to compile pattern matching, inductive type expected{indentExpr type}"
    ...
    let (typeNew, eqRefls) ←
      forallTelescopeReducing motiveType fun targetsNew _ => do
        unless targetsNew.size == targets.size do
          throw_error "invalid number of targets #{targets.size}, motive expects #{targetsNew.size}"
  ...
  if numEqs == 0 then
    ...
    withMVarContext mvarId do
      ...
      if eqDecl.type.isHEq then
        ...
        | none => throw_error "equality expected{indentExpr eqDecl.type}"
        | some (α, a, b) =>
          if (← isDefEq a b) then
            /- Skip equality -/
            ...
            let rec injection (a b : Expr) := do
              ...
              if a.isConstructorApp env && b.isConstructorApp env then
                ...
                if a' != a || b' != b then
                  ...
                  throw_error "dependent elimination failed, stuck at auxiliary equation{indentExpr eqDecl.type}"
            match a, b with
