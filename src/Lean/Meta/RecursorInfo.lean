...
private def checkMotive (declName : Name) (motive : Expr) (motiveArgs : Array Expr) : MetaM Unit :=
  unless motive.isFVar && motiveArgs.all Expr.isFVar do
    throw_error "invalid user defined recursor '{declName}', result type must be of the form (C t), where C is a bound variable, and t is a (possibly empty) sequence of bound variables"
...
private def getMajorPosDepElim (declName : Name) (majorPos? : Option Nat) (xs : Array Expr) (motive : Expr) (motiveArgs : Array Expr)
  ...
  | some majorPos =>
    ...
    else throw_error "invalid major premise position for user defined recursor, recursor has only {xs.size} arguments"
  | none => do
    if motiveArgs.isEmpty then
      throw_error "invalid user defined recursor, '{declName}' does not support dependent elimination, and position of the major premise was not specified (solution: set attribute '[recursor <pos>]', where <pos> is the position of the major premise)"
    let major := motiveArgs.back
    match xs.getIdx? major with
    | some majorPos => pure (major, majorPos, true)
    | none          => throw_error "ill-formed recursor '{declName}'"
  ...
  for i in [:numParams] do
    ...
    | none   => do
      ...
      if localDecl.binderInfo.isInstImplicit then
        ...
        throw_error"invalid user defined recursor '{declName}', type of the major premise does not contain the recursor parameter"
...
private def getIndicesPos (declName : Name) (xs : Array Expr) (majorPos numIndices : Nat) (Iargs : Array Expr) : MetaM (List Nat) := do
  ...
  for i in [:numIndices] do
    ...
    | none   => throw_error "invalid user defined recursor '{declName}', type of the major premise does not contain the recursor index"
...
private def getMotiveLevel (declName : Name) (motiveResultType : Expr) : MetaM Level :=
  ...
  | _                               =>
    throw_error "invalid user defined recursor '{declName}', motive result sort must be Prop or (Sort u) where u is a universe level parameter"
...
private def getUnivLevelPos (declName : Name) (lparams : List Name) (motiveLvl : Level) (Ilevels : List Level) : MetaM (List RecursorUnivLevelPos) := do
  ...
  for p in lparams do
    if motiveLvl == mkLevelParam p then
      ...
      | none   =>
        throw_error "invalid user defined recursor '{declName}', major premise type does not contain universe level parameter '{p}'"
...
private def checkMotiveResultType (declName : Name) (motiveArgs : Array Expr) (motiveResultType : Expr) (motiveTypeParams : Array Expr) : MetaM Unit := do
  if !motiveResultType.isSort || motiveArgs.size != motiveTypeParams.size then
    throw_error "invalid user defined recursor '{declName}', motive must have a type of the form (C : Pi (i : B A), I A i -> Type), where A is (possibly empty) sequence of variables (aka parameters), (i : B A) is a (possibly empty) telescope (aka indices), and I is a constant"
...
private def mkRecursorInfoAux (cinfo : ConstantInfo) (majorPos? : Option Nat) : MetaM RecursorInfo := do
  ...
  forallTelescopeReducing cinfo.type fun xs type => type.withApp fun motive motiveArgs => do
    ...
    if majorPos < numIndices then
      throw_error "invalid user defined recursor '{declName}', indices must occur before major premise"
    ...
    | _ => throw_error "invalid user defined recursor '{declName}', type of the major premise must be of the form (I ...), where I is a constant"
