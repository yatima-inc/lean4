      ...
      if localDecl.isLet then
        ...
        if !(← hasConst (mkBRecOnName indInfo.name)) then
          ...
          if !indIndices.all Expr.isFVar then
            orelse'
              (throw_error "argument #{i+1} was not used because its type is an inductive family and indices are not variables{indentExpr xType}")
              (loop (i+1))
          else if !indIndices.allDiff then
            orelse'
              (throw_error "argument #{i+1} was not used because its type is an inductive family and indices are not pairwise distinct{indentExpr xType}")
              (loop (i+1))
          else
            ...
            | some (index, y) =>
              orelse'
                (throw_error "argument #{i+1} was not used because its type is an inductive family{indentExpr xType}\nand index{indentExpr index}\ndepends on the non index{indentExpr y}")
                (loop (i+1))
            | none =>
              match ← hasBadParamDep? ys indParams with
              | some (indParam, y) =>
                orelse'
                  (throw_error "argument #{i+1} was not used because its type is an inductive datatype{indentExpr xType}\nand parameter{indentExpr indParam}\ndepends on{indentExpr y}")
                  (loop (i+1))
              | none =>
  ...
  if containsRecFn recFnName e then
    Meta.forEachExpr e fun e => do
      if e.isAppOf recFnName then
        throw_error "unexpected occurrence of recursive application{indentExpr e}"
    pure e
  else
    pure e
...
private def withBelowDict {α} (below : Expr) (numIndParams : Nat) (k : Expr → Expr → MetaM α) : MetaM α := do
  ...
  belowType.withApp fun f args => do
    ...
    unless motivePos < args.size do throw_error "unexpected 'below' type{indentExpr belowType}"
        ...
        e.withApp fun f args => do
          if f.isConstOf recFnName then
            ...
            if recArgPos >= args.size then
              throw_error "insufficient number of parameters at recursive application {indentExpr e}"
            ...
            let f ← try toBelow below recArgInfo.indParams.size recArg catch  _ => throw_error "failed to eliminate recursive application{indentExpr e}"
          ...
          let altsNew ← (Array.zip matcherApp.alts matcherApp.altNumParams).mapM fun (alt, numParams) =>
            lambdaTelescope alt fun xs altBody => do
              ...
              unless xs.size >= numParams do
                throw_error "unexpected matcher application alternative{indentExpr alt}\nat application{indentExpr e}"
