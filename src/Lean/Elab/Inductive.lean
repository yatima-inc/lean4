  ...
  try
    ...
    | Expr.sort .. =>
      unless (← isDefEq firstType type) do
        throw_error "resulting universe mismatch, given{indentExpr type}\nexpected type{indentExpr firstType}"
    | _ =>
      throwError "unexpected inductive resulting type"
  catch
            ...
            Term.elabTypeWithAutoBoundImplicit ctorType fun type => do
              ...
              forallTelescopeReducing type fun _ resultingType => do
                unless resultingType.getAppFn == indFVar do
                  throw_error "unexpected constructor resulting type{indentExpr resultingType}"
                unless (← isType resultingType) do
                  throw_error "unexpected constructor resulting type, type expected{indentExpr resultingType}"
              k type
    ...
    let visit (e : Expr) : MetaM TransformStep := do
      ...
      if indFVars.contains f then
        ...
        unless args.size ≥ params.size do
          throw_error "unexpected inductive type occurrence{indentExpr e}"
        for i in [:params.size] do
          let param := params[i]
          let arg := args[i]
          unless (← isDefEq param arg) do
            throw_error "inductive datatype parameter mismatch{indentExpr arg}\nexpected{indentExpr param}"
...
def shouldInferResultUniverse (u : Level) : TermElabM Bool := do
  ...
  if u.hasMVar then
    ...
    | Level.mvar mvarId _ => do
      ...
      pure true
    | _ =>
      throw_error "cannot infer resulting universe level of inductive datatype, given level contains metavariables {mkSort u}, provide universe explicitly"
  else
    pure false
...
def accLevelAtCtor : Level → Level → Nat → Array Level → TermElabM (Array Level)
  ...
  | u,                r, rOffset,   us =>
    ...
    else if r.occurs u  then throw_error "failed to compute resulting universe level of inductive datatype, provide universe explicitly"
    else if rOffset > 0 then throw_error "failed to compute resulting universe level of inductive datatype, provide universe explicitly"
...
def checkResultingUniverse (u : Level) : TermElabM Unit := do
  if bootstrap.inductiveCheckResultingUniverse.get (← getOptions) then
    ...
    if !u.isZero && !u.isNeverZero then
      throw_error "invalid universe polymorphic type, the resultant universe is not Prop (i.e., 0), but it may be Prop for some parameter values (solution: use 'u+1' or 'max 1 u'{indentD u}"
