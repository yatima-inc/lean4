...
def throwInvalidNamedArg {α} (namedArg : NamedArg) (fn? : Option Name) : TermElabM α :=
  withRef namedArg.ref $ match fn? with
    | some fn => throw_error "invalid argument name '{namedArg.name}' for function '{fn}'"
    | none    => throw_error "invalid argument name '{namedArg.name}' for function"
...
def addNamedArg (namedArgs : Array NamedArg) (namedArg : NamedArg) : TermElabM (Array NamedArg) := do
  if namedArgs.any (namedArg.name == ·.name) then
    throw_error "argument '{namedArg.name}' was already set"
...
private def synthesizePendingAndNormalizeFunType : M Unit := do
  ...
  if fType.isForall then
    ...
    | none =>
      ...
      throw_error "function expected at{indentExpr s.f}\nterm has type{indentExpr fType}"
        ...
        if foundIt then
          ...
          for j in [:i] do
            ...
            if prevDecl.userName == xDecl.userName then
              throw_error "invalid field notation, function '{fullName}' has argument with the expected type{indentExpr type}\nbut it cannot be used"
          return (args, namedArgs.push { name := xDecl.userName, val := Arg.expr e })
        if xDecl.binderInfo.isExplicit then
          -- advance explicit argument position
          argIdx := argIdx + 1
    throw_error "invalid field notation, function '{fullName}' does not have argument with type ({baseName} ...) that can be used, it must be explicit or implicit with an unique name"
...
private def mergeFailures {α} (failures : Array (TermElabResult Expr)) : TermElabM α := do
  ...
  throw_error "overloaded, errors {toMessageList msgs}"
