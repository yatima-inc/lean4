    ...
    if (← isExprMVarAssigned instMVar) then
      ...
      unless (← isDefEq oldVal val) do
        ...
        unless (← isDefEq oldValType valType) do
          throw_error "synthesized type class instance type is not definitionally equal to expected type, synthesized{indentExpr val}\nhas type{indentExpr valType}\nexpected{indentExpr oldValType}"
        throw_error "synthesized type class instance is not definitionally equal to expression inferred by typing rules, synthesized{indentExpr val}\ninferred{indentExpr oldVal}"
    else
      unless (← isDefEq (mkMVar instMVar) val) do
        throw_error "failed to assign synthesized type class instance{indentExpr val}"
    pure true
  | LOption.undef    => pure false -- we will try later
  | LOption.none     => throw_error "failed to synthesize instance{indentExpr type}"
...
def synthesizeInst (type : Expr) : TermElabM Expr := do
  ...
  | LOption.undef    => throw_error "failed to synthesize instance{indentExpr type}"
  | LOption.none     => throw_error "failed to synthesize instance{indentExpr type}"
...
def tryPostponeIfHasMVars (expectedType? : Option Expr) (msg : String) : TermElabM Expr := do
  ...
  let some expectedType ← pure expectedType? |
    throw_error "{msg}, expected type must be known"
  let expectedType ← instantiateMVars expectedType
  if expectedType.hasExprMVar then
    tryPostpone
    throw_error "{msg}, expected type contains metavariables{indentExpr expectedType}"
  pure expectedType
...
private def elabUsingElabFnsAux (s : SavedState) (stx : Syntax) (expectedType? : Option Expr) (catchExPostpone : Bool)
  ...
  | []                => do throw_error "unexpected syntax{indentD stx}"
  | (elabFn::elabFns) => do
    try
...
private def elabUsingElabFns (stx : Syntax) (expectedType? : Option Expr) (catchExPostpone : Bool) : TermElabM Expr := do
  ...
  | none         => throw_error "elaboration function for '{k}' has not been implemented{indentD stx}"
...
private def tryCoeSort (α : Expr) (a : Expr) : TermElabM Expr := do
  ...
  try
    ...
    | Exception.error _ msg => throw_error "type expected\n{msg}"
    | _                     => throw_error "type expected"
  ...
  for auto in autoBoundImplicits do
    ...
    for x in xs do
      if (← getMCtx).localDeclDependsOn localDecl x.fvarId! then
        throw_error "invalid auto implicit argument '{auto}', it depends on explicitly provided argument '{x}'"
      ...
      | some val =>
        ...
        if mctx.isWellFormed lctx val then
          ...
          withLCtx mvarDecl.lctx mvarDecl.localInstances do
            throw_error "synthetic hole has already been defined and assigned to value incompatible with the current context{indentExpr val}"
      | none =>
...
def resolveName (n : Name) (preresolved : List (Name × List String)) (explicitLevels : List Level) : TermElabM (List (Expr × List String)) := do
  if let some (e, projs) ← resolveLocalName n then
    unless explicitLevels.isEmpty do
      throw_error "invalid use of explicit universe parameters, '{e}' is a local"
    return [(e, projs)]
...
where process (candidates : List (Name × List String)) : TermElabM (List (Expr × List String)) := do
  if candidates.isEmpty then
    if (← read).autoBoundImplicit && isValidAutoBoundImplicitName n then
      ...
      throw_error "unknown identifier '{Lean.mkConst n}'"
...
def resolveName' (ident : Syntax) (explicitLevels : List Level) : TermElabM (List (Expr × Syntax × List Syntax)) := do
  ...
  | _ => throw_error "identifier expected"
...
def resolveId? (stx : Syntax) (kind := "term") : TermElabM (Option Expr) :=
  ...
  | Syntax.ident _ _ val preresolved => do
    ...
    | []  => pure none
    | [f] => pure (some f)
    | _   =>   throw_error "ambiguous {kind}, use fully qualified name, possible interpretations {fs}"
...
unsafe def evalExpr (α) (typeName : Name) (value : Expr) : TermElabM α :=
  withoutModifyingEnv do
    ...
    unless type.isConstOf typeName do
      throw_error "unexpected type at evalExpr{indentExpr type}"
