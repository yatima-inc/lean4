...
struct vm_env : public external_object {
    environment m_env;
...
obj_res to_vm_env(environment const & env) {
    return new (alloc_heap_object(sizeof(vm_env))) vm_env(env);
}
obj_res vm_environment_empty() {
    return to_vm_env(environment());
}

obj_res vm_environment_contains(b_obj_arg vm_env, b_obj_arg vm_n) {
    return mk_vm_simple(static_cast<bool>(to_env(vm_env).find(name(vm_n, true))));
}
...
name_generator to_name_generator(b_obj_arg o) {
    name_generator ngen;
    ngen.m_prefix = name(cfield(o, 0), true);
    ngen.m_next_idx = unbox(cfield(o, 1));
    return ngen;
}

object_ref to_obj(name_generator const & ngen) {
    return mk_cnstr(0, ngen.m_prefix, object_ref(box(ngen.m_next_idx)));
}
...
vm_obj vm_elaborate_command(b_obj_arg vm_filename, obj_arg vm_cmd, b_obj_arg vm_st) {
    ...
    list_ref<object_ref> vm_lds(cfield(vm_st, 2), true);
    local_level_decls lds;
    name_set lvars;
    for (auto const & vm_ld : vm_lds) {
        auto n = name(cfield(vm_ld.raw(), 0), true);
        lds.insert(n, level(cfield(vm_ld.raw(), 1), true));
        // all local decls are variables in Lean 4
        lvars.insert(n);
    }
    list_ref<object_ref> vm_eds(cfield(vm_st, 3), true);
    local_expr_decls eds;
    name_set vars;
    for (auto const & vm_ed : vm_eds) {
        auto n = name(cfield(vm_ed.raw(), 0), true);
        auto data = cfield(vm_ed.raw(), 1);
        eds.insert(n, mk_local(name(cfield(data, 0), true), n, expr(cfield(data, 1), true), to_binder_info(cfield(data, 2))));
        // all local decls are variables in Lean 4
        vars.insert(n);
    }
    ...
    vm_obj vm_out = mk_option_none();
    {
        ...
        p.reset(snapshot(p.env(), ngen, lds, eds, lvars, vars, includes, options, true, false,
                         parser_scope_stack(), unbox(cfield(vm_st, 6)), pos_info{1, 0}));
        auto ns = name(cfield(vm_st, 7), true);
        ...
        try {
            ...
            for (auto const & ed : s->m_eds.get_entries()) {
                ...
                auto vm_ed = mk_cnstr(0, ed.first, mk_cnstr(0,
                                                                                      local_name_p(ed.second),
                                                                                      local_type_p(ed.second),
                                                                                      object_ref(to_obj(local_info_p(ed.second)))));
            ...
            auto vm_st2 = mk_vm_constructor(0, 8, args);
            vm_out = mk_option_some(vm_st2);
        } catch (exception & e) {
...
vm_obj vm_expr_local(obj_arg vm_name, obj_arg vm_pp_name, obj_arg vm_type, obj_arg vm_binder_info) {
    return mk_local(name(vm_name), name(vm_pp_name), expr(vm_type), to_binder_info(vm_binder_info)).steal();
}
