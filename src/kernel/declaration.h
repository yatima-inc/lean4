...
/* Auxiliary view for accessing inductive_decl constructor fields */
...
public:
    ...
    exprs const & get_types() const { return static_cast<expr const &>(cnstr_obj_ref(raw(), 2)); }
    bool is_meta() const;
};
...
declaration mk_inductive(names const & lparams, nat const & nparams, exprs const & types, bool meta = false);
